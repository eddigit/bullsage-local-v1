...
# ============================================================
# TEMPORARY ENDPOINTS - DELETE AFTER USE
# ============================================================

@router.get("/debug-users")
async def debug_users():
    """Temporary debug endpoint - DELETE AFTER USE"""
    users = await db.users.find({}, {'email': 1, 'is_admin': 1, 'name': 1, '_id': 0}).to_list(100)
...
async def emergency_reset():
    ...
    # Also reset/ensure your specific account is admin
    ...
    existing = await db.users.find_one({'email': 'admin@bullsage.com'})
    if not existing:
        await db.users.insert_one({
            'id': str(uuid.uuid4()),
            'email': 'admin@bullsage.com',
            'name': 'Admin',
            'password': hashed,
            'is_admin': True,
    ...
    admins = await db.users.find({'is_admin': True}, {'email': 1, 'name': 1, '_id': 0}).to_list(10)
    
    return {
        "message": "Admin passwords reset successfully!",
        "new_password": "Admin123!",
        "admins_updated": result.modified_count,
